...
export class AppService {
  // productos:(productosInteface | string)[]
  productos:productosInteface[]
  constructor(){
    this.productos=[
      // "pedro",
  ...
  getProductos(limit:number, skip:number=0):productosInteface[]{
    if(!limit) limit = this.productos.length
    let respuesta:productosInteface[]=this.productos.slice(skip, limit+skip)
    return respuesta
  }

  getProductById(id:number):productosInteface | null{
